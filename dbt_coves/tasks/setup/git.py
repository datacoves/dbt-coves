            ...
            if no_prompt:
                ...
                if not (name and email):
                    raise SetupGitException(
                        f"[yellow]USER_FULLNAME ({name or 'missing'})[/yellow] and [yellow]USER_EMAIL"
                        f"({email or 'missing'})[/yellow] environment variables must be set"
                        f" in order to setup Git with [i]--no-prompt[/i]"
                    )
            else:
